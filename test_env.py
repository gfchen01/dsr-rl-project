...
class SimulationEnv():
    ...
    def _get_coord(self, obj_id, position, orientation, vol_bnds=None, voxel_size=None):
        ...
        if vol_bnds is not None:
            coord = np.round((coord - vol_bnds[:, 0]) / voxel_size).astype(int)
        return coord
    #TODO:
    #Kewen: get the new coordinates and orientation of the obkects
    ...
    def _get_scene_flow_3d(self, old_po_ors):
        ...
        mask = np.zeros([int((x[1] - x[0] + 1e-7) / self.voxel_size) for x in vol_bnds], dtype=int)
    ...
    def _get_scene_flow_2d(self, old_po_or):
        ...
        y = np.round(old_coords_2d[0]).astype(int)
        x = np.round(old_coords_2d[1]).astype(int)
